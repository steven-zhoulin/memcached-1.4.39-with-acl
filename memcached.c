...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    if (transport == tcp_transport && init_state == conn_new_cmd) {
       ...
       int ret = getpeername(sfd, (struct sockaddr *) &c->request_addr, &c->request_addr_size);
       if (ret == 0) {
           char peerip[16];
           inet_ntop(AF_INET, 
                       &((struct sockaddr_in *)&c->request_addr)->sin_addr,
                       peerip, 
                       sizeof(peerip)
                     );
           ...
           if (ACL_ENABLE && !acl_check(peerip)) {
               time_t timep;
               time(&timep);
               printf("deny illegal access from %s | %s", peerip, asctime(gmtime(&timep)));
               return NULL;
           }
       } else {
           perror("getpeername");
...
static void process_command(conn *c, char *command) {
    ...
    } else if (ntokens >= 2 && ntokens <= 4 && (strcmp(tokens[COMMAND_TOKEN].value, "hcmd_flush_all") == 0)) {
        time_t exptime = 0;
        rel_time_t new_oldest = 0;
...
static void usage(void) {
    ...
    printf("-p <num>      TCP port number to listen on (default: 11211)\n"
           ...
           "-Z <file>     specified access control list file <acl file>\n"
...
int main (int argc, char **argv) {
    ...
    while (-1 != (c = getopt(argc, argv,
          ...
          "P:"  /* save PID in file */
          "Z:"  /* access control list file (add by zhoulin) */
          "f:"  /* factor? */
